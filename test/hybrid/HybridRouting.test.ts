...
describe("HybridPool Typescript == Solidity check", function () {
  let alice, feeTo, usdt, usdc, weth, masterDeployer, tridentPoolFactory, router, Pool;
  ...
  async function createHybridPool(A, fee, res0exp, res1exp) {
    ...
    await usdc.transfer(alice.address, getBigNumber("1000000000000000000"));
    await usdt.transfer(alice.address, getBigNumber("1000000000000000000"));
    ...
    await usdt.transfer(pool.address, bnVal0, { from: alice.address });
    await usdc.transfer(pool.address, bnVal1, { from: alice.address });
  ...
  async function checkSwap(pool, poolRouterInfo, swapAmountExp) {
    ...
    const [t0, t1]: Contract[] = swapDirection ? [usdt, usdc] : [usdc, usdt];

    poolRouterInfo.updateReserves(await usdt.balanceOf(pool.address), await usdc.balanceOf(pool.address));
    ...
    let balanceBefore: BigNumber = await t1.balanceOf(alice.address);
    let params: ExactInputSingleParams = {
      amountIn: bnValue,
      amountOutMinimum: 0,
      pool: pool.address,
      tokenIn: t0.address,
      data: ethers.utils.defaultAbiCoder.encode(["address", "address", "bool"], [t0.address, alice.address, false]),
    };
    // this is where router calls bento to transfer bento tokens again
    const tx = await router.connect(alice).exactInputSingle(params);
    let balanceAfter: BigNumber = await t1.balanceOf(alice.address);
